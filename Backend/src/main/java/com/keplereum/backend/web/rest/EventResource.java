...
/**
 * REST controller for managing {@link com.keplereum.backend.domain.Event}.
 */
@RestController
...
public class EventResource {
     ...
     */
    @GetMapping("")
    public List<Event> getAllEvents(@RequestParam(name = "filter", required = false) String filter) {
        if ("validatoraction-is-null".equals(filter)) {
            LOG.debug("REST request to get all Events where validatorAction is null");
            return StreamSupport.stream(eventRepository.findAll().spliterator(), false)
                .filter(event -> event.getValidatorAction() == null)
                .toList();
        }
